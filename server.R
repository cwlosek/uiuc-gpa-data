...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    updateSelectInput(session, "Course", 
                      choices = c(" ", unique(fs1517$Course[fs1517$Subject == input$Subject])), 
                      selected = input$Course)
    
    
    if(nrow(fs1517[fs1517$Subject == input$Subject & fs1517$Course == input$Course, ]) == 1){  
      updateSelectInput(session, "Title", 
                        choices = unique(fs1517$`Course Title`[fs1517$Subject == input$Subject & fs1517$Course == input$Course]))
    }
    else{
      updateSelectInput(session, "Title", 
                        choices = unique(fs1517$`Course Title`[fs1517$Subject == input$Subject & fs1517$Course == input$Course]),
    ...
    updateSelectInput(session, "Professor",
                      choices = c("", fs1517_prof$FirstLast[fs1517_prof$Subject == input$Subject & fs1517_prof$Course == input$Course 
                                                            & fs1517_prof$`Course Title` == input$Title]), 
    ...
    output$gpaPlot = renderPlotly({
      ...
      if(input$display == "Grade Distributions" & input$Subject != "" & input$Course != "" & input$Title != ""){
        plot_ly(y = ~as.numeric(fs1517[fs1517$Subject == input$Subject & 
                                         fs1517$Course == input$Course & 
                                         fs1517$`Course Title` == input$Title, ][16:4]),
                x = ~barplotGradeNames,
                type = "bar",
    ...
    output$scatterplot = renderPlotly({  
      if(input$display == "Scatter Plot (All Courses)"){
        plot_ly(x = ~PerctFour, y = ~GPA, data = fs1517) %>%
          ...
          add_markers(x = ~PerctFour, y = ~GPA, color = ~GPA,
                      hoverinfo = "text",
                      text = ~paste(fs1517$Subject, paste0(fs1517$Course, ":"), fs1517$`Course Title`,
                                    "<br / > GPA:", fs1517$GPA,
                                    "<br / > 4.00's (A+/A) Given:", paste0(fs1517$PerctFour, "%")))
      }
      else{
        return()
      }
    ...
    if(input$display == "Grade Distributions" & input$Subject != "" & input$Course != "" & input$Title != ""){
      output$text1 = renderText({
        ...
        paste("Overall Course GPA:", 
              ...
              paste0(fs1517$PerctFour[fs1517$Subject == input$Subject & 
                                        fs1517$Course == input$Course & 
                                        fs1517$`Course Title` == input$Title], "%"))
        # "|",
      ...
      output$text2 = renderText({
        ...
        paste(c("Recent Professor(s):", paste(fs1517_prof$FirstLast[fs1517_prof$Subject == input$Subject & 
                                                                      fs1517_prof$Course == input$Course & 
                                                                      fs1517_prof$`Course Title` == input$Title], collapse = ", ")))
    ...
    if(input$display == "Grade Distributions" &
      ...
      output$text3 = renderText({
        ...
        paste("Individual GPA for", paste0(input$Professor, ":"), format(fs1517_prof$GPA[fs1517_prof$Subject == input$Subject &
                                                                                    fs1517_prof$Course == input$Course &
                                                                                    fs1517_prof$`Course Title` == input$Title &
                                                                                    fs1517_prof$FirstLast == input$Professor], nsmall = 2))
      })
